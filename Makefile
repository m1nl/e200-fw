...
TARGET ?= e200
SUPPORTED_TARGETS := e200

UBOOT_VERSION = $(shell echo -n "${TARGET}_" && cd u-boot-xlnx && git rev-parse --short HEAD && ([ -n "$(git status --porcelain)" ] && echo '-dirty'))

# Include target specific constants
include scripts/$(TARGET).mk

TARGETS = build/boot.frm build/$(TARGET).frm build/uImage build/rootfs.ext4 build/rootfs.cpio.gz sdimg jtag-bootstrap

ifeq ($(findstring $(TARGET),$(SUPPORTED_TARGETS)),)
all:
	@echo "Invalid `TARGET variable ; valid values are: e200" &&
	exit 1
else
...
u-boot-xlnx/u-boot u-boot-xlnx/u-boot.dtb u-boot-xlnx/tools/mkimage: TOOLCHAIN
...
linux/arch/arm/boot/dts/%.dtb: TOOLCHAIN linux/arch/arm/boot/dts/%.dts linux/arch/arm/boot/dts/*.dtsi
...
buildroot/output/images/rootfs.ext4 buildroot/output/images/rootfs.cpio.gz:
	@echo device-fw $(VERSION)> $(CURDIR)/buildroot/board/$(TARGET)/VERSIONS
	@$(foreach dir,$(VSUBDIRS),echo $(dir) $(shell cd $(dir) && git rev-parse --short HEAD && ([ -n "$(git status --porcelain)" ] && echo '-dirty')) >> $(CURDIR)/buildroot/board/$(TARGET)/VERSIONS;)
	make -C buildroot ARCH=arm zynq_$(TARGET)_defconfig
...
sdimg: build/fsbl.elf build/system_top.bit build/u-boot.elf build/zynq-$(TARGET).dtb build/uboot-env.txt build/uImage build/rootfs.cpio.gz u-boot-xlnx/tools/mkimage
	rm -r $(SDIMGDIR) || true
	mkdir $(SDIMGDIR)
	...
	u-boot-xlnx/tools/mkimage -A arm -T ramdisk -C gzip -d $(SDIMGDIR)/ramdisk.image.gz $(SDIMGDIR)/uramdisk.image.gz
